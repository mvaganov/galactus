	...
	public class SDK_InputSimulator : MonoBehaviour
	{
		...
		public static GameObject FindInScene()
		{
			if (cachedCameraRig == null && !destroyed)
			{
				...
				if (!cachedCameraRig)
				{
					VRTK_Logger.Error(VRTK_Logger.GetCommonMessage(VRTK_Logger.CommonMessageKeys.REQUIRED_COMPONENT_MISSING_FROM_SCENE, "[VRSimulator_CameraRig]", "SDK_InputSimulator", ". check that the `VRTK/Prefabs/VRSimulatorCameraRig` prefab been added to the scene."));
				}
			}
		...
		protected virtual void OnEnable()
		{
			...
			leftHand.Find("Hand").GetComponent<Renderer>().material.color = leftHandColor;
			rightHand.Find("Hand").GetComponent<Renderer>().material.color = rightHandColor;
