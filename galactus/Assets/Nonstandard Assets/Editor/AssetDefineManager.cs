...
public class AssetDefineManager : AssetPostprocessor
{
	...
	{
		// prevents weird error logs for broken platforms
		const string noteToSelf = "__AssetDefineManagerPreventWeirdErrorsPlease";
		bool ivedonethisbefore = PlayerPrefs.GetString (noteToSelf) == Application.dataPath;
			...
			for (int i = 0; i < fileCodes.Length; ++i) {
				...
				if(fileName == def.assetDetectionFile)
				{
					if (def.IsValid) {        //this is an asset we are tracking for defines
						def.AddAllDefines (ivedonethisbefore); // this will cause weird error logs for platforms that can't be predicted
						ivedonethisbefore = true;
					}
				}
			}
		}
		PlayerPrefs.SetString (noteToSelf, Application.dataPath); // saves to windows registry... hurray for global variables.
	}
