...
/// Latest version at: https://pastebin.com/xFUD4tk2 -- added: Working with _BodyAlign (2017/10/07)
...
public class MovingEntity : MovingEntityBase {
    ...
    public enum HowToDealWithWalls {smoothMove, smoothMoveAll, allowWallSticking}
    [HideInInspector]
    ...
    public float DistanceTo(Vector3 loc) {
        // TODO factor in collider offset...
    ...
    public void FollowPositionOnPlatform() {
        ...
        if(transformOnPlatform != null && wallGroundInteraction.stickToGround == HowToDealWithGround.withPlaceholder &&
    ...
    protected override void ApplyMove(float acceleration) {
        ...
        if(IsMovingIntentionally) {
            ...
            } else if(wallGroundInteraction.pressAgainstWalls != HowToDealWithWalls.allowWallSticking && wallForce != Vector3.zero) {
                float againstMove = Vector3.Dot(-wallForce, MoveDirection);
                against = wallForce * againstMove;
            }
            if(against != Vector3.zero) {
                MoveDirection += against;
    ...
    public WallGroundInteraction wallGroundInteraction = new WallGroundInteraction(45, HowToDealWithGround.withPlaceholder, HowToDealWithWalls.smoothMove);
    ...
    public struct WallGroundInteraction {
        ...
        public HowToDealWithGround stickToGround;
        ...
        public HowToDealWithWalls pressAgainstWalls;
        [Tooltip("The maximum angle the player can move forward at while standing on the ground")]
        public float MaxWalkAngle;
        public WallGroundInteraction(float maxWalkAngle, HowToDealWithGround groundMove, HowToDealWithWalls wallMove){
    ...
    void FixedUpdate() {
        ...
        if(gravity.application != GravityState.none && transformOnPlatform && transformOnPlatform.parent != null &&
            wallGroundInteraction.stickToGround == HowToDealWithGround.withPlaceholder) {
    ...
    void OnCollisionStay(Collision c) {
        ...
        } else if(wallGroundInteraction.pressAgainstWalls == HowToDealWithWalls.smoothMoveAll
            || (wallGroundInteraction.pressAgainstWalls == HowToDealWithWalls.smoothMove && c.gameObject.GetComponent<Rigidbody>() == null)) {
